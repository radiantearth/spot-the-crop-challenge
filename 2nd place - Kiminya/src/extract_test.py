...
OUTPUT_DIR = sys.argv[2] #data/s2/test
...
df_test = load_df(f'{FOLDER_BASE}_test_labels')
df_test['date'] = df_test.datetime.astype(np.datetime64)
bands = [k for k,v in DOWNLOAD_S2.items() if v==True]

def extract_s2_test(tile_ids):
  ...
  for tile_id in tqdm(tile_ids):
      df_tile = df_test[df_test['tile_id']==tile_id]
  ...
  df = pd.DataFrame(dict(field_id=fields,tile_id=tiles,dates=dates))
  return df

tile_ids = sorted(df_test.tile_id.unique())
...
results = []
for t in tasks:
    results.append(pool.apply_async(extract_s2_test, t))
...
df_test_meta = pd.concat(all_results)
df_test_meta['field_id'] = df_test_meta.field_id.astype(np.int32)
df_test_meta['tile_id'] = df_test_meta.field_id.astype(np.int32)
df_test_meta = df_test_meta.sort_values(by=['field_id']).reset_index(drop=True)
df_test_meta.to_pickle(f'{OUTPUT_DIR}/field_meta_test.pkl')


print(f'Test bands saved to {OUTPUT_DIR}')
print(f'Test metadata saved to {OUTPUT_DIR}/field_meta_test.pkl')